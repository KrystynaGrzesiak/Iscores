...
If you have access to the original dataset before imputation, you can also use the energy distance as an additional evaluation metric. Our package provides an easy-to-use wrapper `edistance()` around the `energy::eqdist.e` function from the energy package. You can use it providing complete and imputed datasets:

```{r}
# edistance(X, impute_mice_rf(X))
# 
# edistance(X, impute_zero(X))
```
